...
Ви можете налаштувати інтерфейс користувача RainbowKit, щоб він відповідав вашому брендингу. Ви можете вибрати з кількох попередньо визначених акцентних кольорів та конфігурацій радіусу кордонів.
...
Функція теми повертає об'єкт теми. Ви можете передати об'єкт до пропа `theme` компонента `RainbowKitProvider`.
...
Якщо хочете, ви можете передати `accentColor`, `accentColorForeground`, `borderRadius`, `fontStack` та `overlayBlur` для їх налаштування.
...
Кожна тема також пропонує декілька пресетів акцентних кольорів (`blue`, `green`, `orange`, `pink`, `purple`, `red`), які можна розповсюдити в об'єкт опцій. Наприклад, щоб використати пресет акцентного кольору `pink`
...
> Нагадування: доступні значення радіусу кордону: `large` (за замовчуванням), `medium`, `small` та `none`.

#### Набір шрифтів

За замовчуванням, `fontStack` встановлено на `rounded`. А ось як ви можете використовувати конфігурацію `fontStack`.
...
За замовчуванням, `overlayBlur` встановлено на `none`. А ось як ви можете використати налаштування `overlayBlur`.
...
#### Комбінуйте та підбирайте

Ось кілька різних способів, як ви можете використовувати різні теми з пропсами `accentColor`, `borderRadius` та `fontStack` разом.
...
```tsx
import { RainbowKitProvider, lightTheme } from '@merkletrade/rainbowkit';
...
Якщо ваш додаток використовує стандартний медіа-запит `prefers-color-scheme: dark` для перемикання між світлим і темним режимами, ви можете додатково надати динамічний об'єкт теми, що містить значення `lightMode` та `darkMode`.
