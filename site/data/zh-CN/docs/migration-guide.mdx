...
现在支持新的EIP-6963连接标准的钱包（包括Rainbow、MetaMask等）将自动显示在连接钱包体验的`Installed`部分。 这确保用户始终可以找到他们喜欢的钱包，并且在不发生冲突或需要后备按钮的情况下连接到dApps。
...
您可以通过传递导入的或自定义钱包连接器到`wallets`来创建一个自定义钱包列表。 您可以通过传递导入的或自定义钱包连接器到`wallets`来创建一个自定义钱包列表。 不再需要实例化钱包连接器并传递`projectId`和`chains`。 
...
参考我们的 [Next.js Webpack Config](https://github.com/merkletrade/rainbowkit/blob/main/examples/with-next/next.config.js) 和 [Create React App polyfills](https://github.com/merkletrade/rainbowkit/blob/main/examples/with-create-react-app/src/polyfills.ts) 示例，以获得你的项目配置指导。
...
每个dApp现在必须提供一个 [WalletConnect Cloud](https://cloud.walletconnect.com/) `projectId`以启用WalletConnect v2。 这必须在2023年6月28日关闭WalletConnect v1桥接服务器之前完成。  如果`projectId`未指定，RainbowKit将默认首选所有钱包为 WalletConnect v1。 
