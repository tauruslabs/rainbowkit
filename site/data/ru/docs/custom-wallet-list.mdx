...
Вы можете импортировать отдельные кошельки из `'@merkletrade/rainbowkit/wallets'` вместе с функцией `connectorsForWallets` для построения собственного списка кошельков с необходимыми коннекторами. Таким образом, у вас есть полный контроль над тем, какие кошельки отображать и в каком порядке.
