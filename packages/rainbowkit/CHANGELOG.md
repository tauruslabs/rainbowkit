...
- 24b5a88: Improved support for `NodeNext` module resolution to resolve `"@merkletrade/rainbowkit"' has no exported member 'RainbowKitProvider'` TypeScript warnings
...
- 02e796c0: SafePal Wallet Support
  ...
  **Example usage**
  ...
  import { safepalWallet } from "@merkletrade/rainbowkit/wallets";
  const { wallets } = getDefaultWallets({ appName, chains });
  const connectors = connectorsForWallets([
    ...wallets,
    {
...
- efb8566e: SubWallet Support
  ...
  **Example usage**
  ...
  import { subWallet } from "@merkletrade/rainbowkit/wallets";
...
- 4b7a44c8: Okto Support
  ...
  **Example usage**
  ...
  import { oktoWallet } from "@merkletrade/rainbowkit/wallets";
...
- 2c8abbb2: Zeal Support
  ...
  **Example usage**
  ...
  import { zealWallet } from "@merkletrade/rainbowkit/wallets";
  const { wallets } = getDefaultWallets({ appName, chains });
  const connectors = connectorsForWallets([
    ...wallets,
    {
...
- e41103fb: CLV Wallet Support
  ...
  **Example usage**
  ...
  import { clvWallet } from "@merkletrade/rainbowkit/wallets";
  const { wallets } = getDefaultWallets({ appName, chains });
  const connectors = connectorsForWallets([
    ...wallets,
    {
...
- b0022aea: Desig Wallet support
  ...
  **Example usage**
  ...
  import { desigWallet } from "@merkletrade/rainbowkit/wallets";
  const { wallets } = getDefaultWallets({ appName, chains, projectId });
  const connectors = connectorsForWallets([
    ...wallets,
    {
...
- b37f5d68: RainbowKit is now localized in 13 languages and counting 🌎
  ...
  If you would like to see support for an additional language, please open a [GitHub Discussion](https://github.com/merkletrade/rainbowkit/discussions/new?category=ideas) and we'll work to support it as soon as possible.
...
- 1b4f142e: BitKeep Support
  ...
  **Example usage**
  ...
  import { bitKeepWallet } from "@merkletrade/rainbowkit/wallets";
  const { wallets } = getDefaultWallets({ appName, projectId, chains });
  const connectors = connectorsForWallets([
    ...wallets,
    {
...
- 5ddc813: Fix package.json exports field backwards compatibility for `@merkletrade/rainbowkit/wallets`

## 0.7.0

### Minor Changes

- 2e6bb8f: Support tree shaking of individual wallets by exposing them as separate imports via the new `@merkletrade/rainbowkit/wallets` entry point.
...
- 2e6bb8f: The Steakwallet backwards compatibility layer has been removed. Omni should be used instead, available via the new `@merkletrade/rainbowkit/wallets` entry point.
...
- c944ddc: Allow custom wallets to automatically hide themselves based on the availability of other wallets in the list. This can be achieved via the new optional `hidden` function on the `Wallet` type.
  ...
  **Example usage**
  ...
  ```ts
  import type Wallet from "@merkletrade/rainbowkit";
...
- 12912b3: Fix the `@merkletrade/rainbowkit/styles.css` import for Jest, and other tooling that doesn't support the `exports` field in `package.json`.

  Jest currently has issues when importing RainbowKit styles due to lack of support for the `exports` field, which we use to publicly alias `@merkletrade/rainbowkit/dist/index.css` as `@merkletrade/rainbowkit/styles.css`. To fix this, we now include a `styles.css` _directory_ in the RainbowKit package containing a `package.json` file whose `main` field points to `"../dist/index.css"`.
...
- 737a1d6: Added support for authentication.
  ...
  RainbowKit now provides first-class support for [Sign-In with Ethereum](https://login.xyz) and [NextAuth.js](https://next-auth.js.org) via the `@merkletrade/rainbowkit-siwe-next-auth` package, as well as lower-level APIs for integrating with custom back-ends and message formats.
...
- b785e31: Make `getDefaultWallets` return an object containing `connectors` and `wallets`
  ...
  **Migration guide**
  ...
  ```diff
  -import { getDefaultWallets, connectorsForWallets } from '@merkletrade/rainbowkit';
  +import { getDefaultWallets } from '@merkletrade/rainbowkit';
...
- 353d2ef: Add support for custom `accentColor` values to built-in themes and add an `accentColorForeground` option to support custom text colors when rendered on top of the accent color
  ...
  **Example usage**
  ...
  ```tsx
  import { RainbowKitProvider, darkTheme } from "@merkletrade/rainbowkit";
...
- eaa5bf6: Support custom wallet groups
  ...
  **Example usage**
  ...
  ```tsx
  import { wallet, WalletList } from "@merkletrade/rainbowkit";
